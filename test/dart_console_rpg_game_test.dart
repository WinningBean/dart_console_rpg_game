...
void main() {
  group('Character', () {
    ...
    test('Character attack test', () {
      ...
      expect(
        player.health,
        lessThanOrEqualTo(player.maxHealth),
      ); // 방어 후 체력은 최대 체력을 초과하지 않아야 함
    });
  });

  group('GameLoader', () {
    // 몬스터 로드 테스트
    test('GameLoader loadMonsters test', () {
      final monsters = GameLoader.loadMonsters('files/monsters.txt', 50);

      expect(monsters, isNotEmpty);
      expect(monsters.first, isA<Monster>());
    });
  });
}
